...
<div class="content-wrapper">
  <div  style="display: flex; justify-content: center">
    <v-app>
      ...
      >
        <template v-slot:top>
          <h3 style="display: flex; justify-content: center">Category List</h3>
          <v-toolbar flat>
            <template>
              <v-btn
                small
                      ...
                      <v-form ref="form">
                        <v-col cols="12" sm="6" md="8">
                          <v-text-field
                            ...
                            :rules="[
                              (v) => v.length >= 3 || 'This field is required',
                            ]"
                            label="Category name"
                          ></v-text-field>
                        </v-col>
...
export default {
  ...
  methods: {
    editItem(item) {
      this.editedIndex = this.GetCategory.indexOf(item);
      this.editedItem = Object.assign({}, item);
      this.Editdialog = true;
    },

    deleteItem(item) {
      this.DeleteIndex = this.GetCategory.indexOf(item);
    ...
    deleteItemConfirm() {
      ...
      if (this.multiSelect.length>0) {
        axios.post("/admin/multi_delete", this.multiSelect)
        .then((response) => {
          this.$store.dispatch("AllCategory");
          this.multiSelect=""
        ...
        axios
          .post("/admin/delete_category", this.DeleteItems)
          .then((response) => {
            this.$store.dispatch("AllCategory");
            // this.GetCategory.splice(this.DeleteIndex, 1);
            this.dialogDelete = false;
    ...
    save() {
      ...
      if (this.$refs.form.validate()) {
        axios
          .post("/admin/add_category", this.AddItem)
          .then((res) => {
    ...
    Update() {
      ...
      if (this.$refs.form.validate()) {
        axios
          .post("/admin/update_category", this.editedItem)
          .then((res) => {
            //   this.GetCategory.unshift(res.data);
            this.$store.dispatch("AllCategory");
